...
class OptimizerWrapper():
    ...
    def __init__(self, model, args):
        ...
        # create optimizer
        if (self.op_flag == "Adam") or (self.op_flag == "pcAdam"):
            if self.l2_penalty > 0:
                self.optimizer = torch_optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), 
                                                  lr=self.lr, 
            ...
            if self.l2_penalty > 0:
                self.optimizer = torch_optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), 
                                                  lr=self.lr, 
                                                  eps=self.eps,
                                                  weight_decay=self.l2_penalty)
            else:
