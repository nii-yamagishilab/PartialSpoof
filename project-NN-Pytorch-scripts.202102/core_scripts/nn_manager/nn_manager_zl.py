...
def f_run_one_epoch(args,
                    ...
                    target_norm_method = None, dev_flag=False):
    """
    f_run_one_epoch: 
    ...
    for data_idx, (data_in, data_tar, data_info, idx_orig) in \
        ...
        if args.model_forward_with_target:
            ...
            if args.model_forward_with_file_name:
                if dev_flag:
                    data_gen = pt_model(data_in, data_info, dev_flag)
                else:    
        ...
        if hasattr(pt_model, 'loss'):
            ...
            if(args.loss_weight == 'dwa'):
                loss_computed = pt_model.loss(data_gen, data_tar, epoch_idx) # not finish edit
            else:
            ...
            if(args.loss_weight == 'dwa'):
                loss_computed = loss_wrapper.compute(data_gen, normed_target, epoch_idx)
            else:
        ...
        if optimizer is not None:
            # zlin Back-propgation using the PCGrad
            if (args.optimizer == 'pcAdam' ):
                pc_adam.pc_backward(loss_computed[0])
...
def f_train_wrapper(args, pt_model, loss_wrapper, device, \
    ...
    if torch.cuda.device_count() > 1 :
        nii_nn_tools.f_model_show(pt_model, do_model_def_check=False)
    else:
        ...
        if val_dataset_wrapper is not None:
            ...
            with torch.no_grad():
                f_run_one_epoch(args, pt_model, loss_wrapper, \
                                ...
                                epoch_idx, None, normtarget_f, True)  #last True used to notice here is dev
